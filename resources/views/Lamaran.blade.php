...
<head>
    ...
    <title>Lamaran Anda - KerjaKuy</title>
...
<body class="app-shell font-body-md text-body-md text-on-surface antialiased">
    ...
    <section class="relative overflow-hidden bg-gradient-to-br from-primary via-primary-container to-secondary py-16 md:py-20 px-margin-mobile md:px-margin-desktop text-center">
        ...
        <div class="max-w-3xl mx-auto relative z-10">
            ...
            <p class="font-body-lg text-body-lg text-primary-fixed-dim mb-lg">Pantau status lamaran kerja Anda dengan mudah.</p>

            <div class="relative w-full max-w-2xl mx-auto">
                <span class="material-symbols-outlined absolute left-md top-1/2 -translate-y-1/2 text-outline">search</span>
                <input type="text" placeholder="Cari posisi atau perusahaan..." class="search-input w-full pl-[56px] pr-md py-[16px] rounded-full border border-outline-variant focus:ring-2 focus:ring-secondary text-on-surface shadow-md font-body-md text-body-md outline-none bg-surface-container-lowest">
            </div>
        </div>
    </section>
    ...
    <main class="flex-grow flex flex-col items-center px-margin-mobile md:px-margin-desktop py-xl max-w-7xl mx-auto w-full">
        <div class="bg-surface-container-lowest p-xs rounded-full shadow-sm mb-xl inline-flex border border-surface-variant flex-wrap justify-center gap-2">
            <button class="tab-btn px-lg py-sm rounded-full font-label-md text-label-md text-on-surface-variant hover:bg-surface-container" data-tab="semua">Semua</button>
            <button class="tab-btn tab-active px-lg py-sm rounded-full font-label-md text-label-md bg-primary-container text-on-primary" data-tab="diproses">Diproses</button>
            <button class="tab-btn px-lg py-sm rounded-full font-label-md text-label-md text-on-surface-variant hover:bg-surface-container" data-tab="diterima">Diterima</button>
            <button class="tab-btn px-lg py-sm rounded-full font-label-md text-label-md text-on-surface-variant hover:bg-surface-container" data-tab="ditolak">Ditolak</button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-gutter w-full max-w-7xl cards-container">
            ...
            <article class="card card-soft bg-surface-container-lowest rounded-xl p-6 border border-outline-variant shadow-sm relative overflow-hidden group" data-status="{{ $uiStatus }}">
                ...
                <div class="flex items-start justify-between mb-4">
                    <div class="flex gap-4">
                        <div class="w-12 h-12 rounded-lg bg-surface-container flex items-center justify-center overflow-hidden border border-outline-variant">
                            ...
                            <img alt="Logo {{ $company }}" class="w-full h-full object-cover" src="{{ asset('storage/' . $lamaran->lowongan->perusahaan->foto_profil) }}">
                            @else
                            <span class="material-symbols-outlined text-outline" style="font-variation-settings: 'FILL' 0;">apartment</span>
                            @endif
                        </div>
                        <div>
                            <h3 class="card-title text-headline-md font-headline-md text-on-surface mb-0.5">{{ $lamaran->lowongan->posisi_pekerjaan }}</h3>
                            <p class="card-company text-body-sm font-medium text-on-surface-variant">{{ $company }}</p>
                        </div>
                    </div>

                    <span class="px-3 py-1 rounded-full text-label-sm font-bold {{ $uiStatus === 'diterima' ? 'bg-tertiary-fixed text-on-tertiary-fixed-variant' : ($uiStatus === 'ditolak' ? 'bg-error-container text-on-error-container' : 'bg-secondary-container text-on-secondary-container') }}">
                        {{ ucfirst($uiStatus) }}
                    </span>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex justify-between items-center gap-4 flex-wrap">
                        <span class="text-body-sm text-outline">Dilamar pada {{ optional($lamaran->created_at)->format('d M Y') }}</span>
                        <a href="{{ route('lowongan.detail', $lamaran->lowongan->id) }}" class="py-2.5 px-4 border border-primary text-primary hover:bg-primary-container/10 rounded-lg text-label-md transition-colors">
                            Lihat Detail
                        </a>
                    </div>
                </div>
