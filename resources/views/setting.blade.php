...
<header class="sticky top-0 z-50 bg-surface-container-lowest shadow-sm h-20 flex items-center">
    <div class="flex justify-between items-center px-margin-mobile md:px-margin-desktop w-full">
        <div class="flex items-center gap-md md:gap-xl">
            ...
            <a href="{{ route('home') }}" class="text-headline-md font-headline-md font-bold text-on-surface">KerjaKuy</a>
    ...
    <aside class="fixed inset-y-0 left-0 z-40 w-72 bg-primary-container dark:bg-on-primary-fixed flex flex-col pt-lg pb-xl transform -translate-x-full transition-transform duration-300 ease-in-out md:relative md:translate-x-0 md:transform-none shrink-0">
        ...
        <nav class="flex-1 space-y-base px-sm">
            ...
            <div class="md:hidden border-b border-white/10 pb-sm mb-sm px-sm space-y-base">
                ...
                <a class="flex items-center gap-sm px-md py-sm rounded-lg text-on-primary-container hover:bg-white/5 transition-all" href="{{ route('home') }}">
                    <span class="material-symbols-outlined">work</span>
                    <span class="font-label-md text-label-md">Lowongan Kerja</span>
                </a>
                <a class="flex items-center gap-sm px-md py-sm rounded-lg text-on-primary-container hover:bg-white/5 transition-all" href="{{ url('/lamaran-anda') }}">
                    <span class="material-symbols-outlined">assignment</span>
                    <span class="font-label-md text-label-md">Lamaran Anda</span>
                </a>
                <a class="flex items-center gap-sm px-md py-sm rounded-lg text-on-primary-container hover:bg-white/5 transition-all" href="{{ route('pelamar.wawancara') }}">
            ...
            <!-- Akun (Active) -->
            <a class="flex items-center gap-sm px-md py-sm rounded-lg bg-secondary text-white font-bold transition-all active:scale-95" href="{{ route('pelamar.settings') }}">
                <span class="material-symbols-outlined">person</span>
                <span class="font-label-md text-label-md">Akun</span>
            </a>
            <!-- CV -->
            <a class="flex items-center gap-sm px-md py-sm rounded-lg text-on-primary-container hover:bg-white/5 transition-all active:scale-95" href="{{ route('cv.index') }}">
    ...
    <section class="flex-1 bg-surface py-lg px-margin-mobile md:py-xl md:px-margin-desktop overflow-y-auto">
        <div class="max-w-3xl">
            <header class="mb-lg border-b border-outline-variant/30 pb-md">
                ...
                <p class="text-on-surface-variant font-body-sm mt-xs">Kelola informasi profil dan pengaturan akun Anda.</p>
            </header>
            ...
            <form action="{{ route('pelamar.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="bg-surface-container-lowest border border-outline-variant/20 rounded-xl p-md shadow-sm mb-lg flex flex-col sm:flex-row items-center text-center sm:text-left gap-md sm:gap-lg">
                    <div class="relative group flex-shrink-0">
                        <div class="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-md cursor-pointer" onclick="document.getElementById('inputFoto').click()">
                            <img id="profile-preview" alt="Profile Photo" class="w-full h-full object-cover" src="{{ $pelamar->foto_profil ? asset('storage/' . $pelamar->foto_profil) : 'https://cdn-icons-png.flaticon.com/512/847/847969.png' }}"/>
                        </div>
                        ...
                        <input type="file" name="foto_profil" id="inputFoto" class="hidden" onchange="previewImg()"/>
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-md">
                    ...
                    <div class="space-y-xs group">
                        <label class="text-label-md font-label-md text-on-surface-variant px-base">Nama Lengkap</label>
                        <div class="relative">
                            <input name="nama_lengkap" class="w-full h-12 px-md pr-12 bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface" type="text" value="{{ old('nama_lengkap', $pelamar->nama_lengkap) }}"/>
                    ...
                    <!-- Keahlian -->
                    <div class="space-y-xs group">
                        <label class="text-label-md font-label-md text-on-surface-variant px-base">Keahlian (Pisahkan dengan koma)</label>
                        <div class="relative">
                            <input name="keahlian" class="w-full h-12 px-md pr-12 bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface" type="text" value="{{ old('keahlian', $keahlianString) }}"/>
                            <button type="button" class="edit-btn absolute right-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-outline hover:text-secondary transition-colors cursor-pointer">edit</button>
                        </div>
                    </div>
                    <!-- Username -->
                    <div class="space-y-xs group">
                        ...
                        <div class="relative">
                            <input name="username" class="w-full h-12 px-md pr-12 bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface" type="text" value="{{ old('username', $pelamar->username) }}"/>
                            <button type="button" class="edit-btn absolute right-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-outline hover:text-secondary transition-colors cursor-pointer">edit</button>
                        </div>
                    </div>
                    <!-- Email -->
                    <div class="space-y-xs group">
                        ...
                        <div class="relative">
                            <input name="email" class="w-full h-12 px-md pr-12 bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface" type="email" value="{{ old('email', $pelamar->email) }}"/>
                            <button type="button" class="edit-btn absolute right-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-outline hover:text-secondary transition-colors cursor-pointer">edit</button>
                        </div>
                    </div>
                    <!-- No. Telfon -->
                    <div class="space-y-xs group">
                        <label class="text-label-md font-label-md text-on-surface-variant px-base">No. Telfon</label>
                        <div class="relative">
                            <input name="no_telp" class="w-full h-12 px-md pr-12 bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface" type="tel" value="{{ old('no_telp', $pelamar->no_telp) }}"/>
                            <button type="button" class="edit-btn absolute right-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-outline hover:text-secondary transition-colors cursor-pointer">edit</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-xl flex flex-col-reverse sm:flex-row justify-end gap-sm sm:gap-md">
                    <a href="{{ route('pelamar.settings') }}" class="flex items-center justify-center w-full sm:w-auto px-lg h-12 rounded-lg font-label-md text-on-surface-variant hover:bg-surface-container transition-all">Batalkan</a>
...
<!-- Hapus Akun Modal -->
<div id="deleteAccountModal" class="fixed inset-0 z-50 items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm hidden">
    <div class="bg-surface-container-lowest p-lg rounded-xl shadow-xl w-full max-w-md mx-margin-mobile md:mx-0 relative flex flex-col border border-outline-variant/30">
        <!-- Close Button -->
        ...
        <p class="text-on-surface-variant font-body-sm mb-lg border-b border-outline-variant/30 pb-md">
            Apakah Anda yakin ingin menghapus akun ini? Tindakan ini <strong class="text-error">tidak dapat dibatalkan</strong> dan seluruh data profil, lamaran, CV, serta portofolio Anda akan dihapus secara permanen.
        </p>
        ...
        <form action="{{ route('pelamar.destroy') }}" method="POST">
            ...
            <div class="flex justify-end gap-md">
                ...
                <button type="submit" class="px-lg h-12 bg-error text-white rounded-lg font-label-md shadow-md hover:bg-opacity-90 transition-all active:scale-95">Ya, Hapus Akun</button>
            </div>
        </form>
    </div>
</div>
