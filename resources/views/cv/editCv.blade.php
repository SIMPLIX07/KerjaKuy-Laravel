                ...
                @for ($i = 0; $i < 3; $i++)
                    ...
                    <div class="mb-4 p-4 border rounded-lg">
                        <input type="text" name="pendidikan[{{ $i }}][tingkat]"
                            value="{{ $p->tingkat ?? '' }}" placeholder="Jenjang"
                            class="w-full border rounded px-3 py-2 mb-2">

                        <input type="text" name="pendidikan[{{ $i }}][universitas]"
                            value="{{ $p->universitas ?? '' }}" placeholder="Institusi"
                            class="w-full border rounded px-3 py-2 mb-2">

                        <input type="text" name="pendidikan[{{ $i }}][jurusan]"
                            value="{{ $p->jurusan ?? '' }}" placeholder="Jurusan"
                ...
                @for ($i = 0; $i < 3; $i++)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                        <input type="text" name="skill[{{ $i }}][skill]"
                            value="{{ $cv->skills[$i]->skill ?? '' }}" placeholder="Nama Skill"
                            class="w-full border border-[#1FACA2] rounded-lg px-3 py-2">

                        <input type="number" name="skill[{{ $i }}][kemampuan]"
                            value="{{ $cv->skills[$i]->kemampuan ?? '' }}" placeholder="Kemampuan (1-100)"
                            class="w-full border border-[#1FACA2] rounded-lg px-3 py-2">
                    </div>
                @endfor
            </div>
                ...
                @for ($i = 0; $i < 3; $i++)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                        <input type="text" name="pengalaman[{{ $i }}][pengalaman]"
                            value="{{ $cv->pengalamans[$i]->pengalaman ?? '' }}" placeholder="Nama Pengalaman"
                            class="w-full border border-[#1FACA2] rounded-lg px-3 py-2">

                        <input type="text" name="pengalaman[{{ $i }}][durasi]"
                            value="{{ $cv->pengalamans[$i]->durasi ?? '' }}" placeholder="Durasi"
                            class="w-full border border-[#1FACA2] rounded-lg px-3 py-2">
                    </div>
                @endfor
            </div>
