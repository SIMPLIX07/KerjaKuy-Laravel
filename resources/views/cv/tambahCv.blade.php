...
<body class="bg-surface text-on-surface font-body-md selection:bg-secondary-container selection:text-on-secondary-container">
    ...
    <header class="bg-primary dark:bg-on-primary-fixed docked full-width top-0 shadow-md z-50">
        <div class="flex justify-between items-center px-margin-desktop w-full h-20 max-w-7xl mx-auto bg-gradient-to-r from-primary via-primary-container to-secondary">
            <div class="flex items-center gap-xl">
                <a class="text-headline-md font-headline-md font-bold text-white tracking-tight" href="{{ route('home') }}">KerjaKuy</a>
    ...
    <main class="min-h-screen pb-xl">
        ...
        <section class="vibrant-gradient pt-32 pb-xl px-margin-desktop text-center text-white relative overflow-hidden">
            ...
            <div class="max-w-3xl mx-auto relative z-10">
                ...
                <p class="font-body-lg text-body-lg text-white/90">Lengkapi profil Anda dan dapatkan peluang karir impian di KerjaKuy. CV yang profesional adalah langkah pertama menuju kesuksesan.</p>
            </div>
        </section>
                ...
                <div class="bg-white p-md md:p-lg rounded-xl border border-outline-variant/30 form-card-shadow">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-md">
                        <div class="space-y-base">
                            ...
                            <input class="w-full p-md border border-outline-variant rounded-lg focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all bg-white" placeholder="Masukkan posisi profesional Anda" type="text" name="title" required value="{{ old('title') }}">
                        </div>
                        <div class="space-y-base">
                            <label class="font-label-md text-label-md text-outline">Umur</label>
                            <input class="w-full p-md border border-outline-variant rounded-lg focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all bg-white" placeholder="25" type="number" name="umur" required value="{{ old('umur') }}">
                        </div>
                        <div class="space-y-base">
                            <label class="font-label-md text-label-md text-outline">Kontak (Nomor Telepon/Email)</label>
                            <input class="w-full p-md border border-outline-variant rounded-lg focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all bg-white" placeholder="0812-3456-7890" type="text" name="kontak" required value="{{ old('kontak') }}">
                        </div>
                        <div class="space-y-base">
                            <label class="font-label-md text-label-md text-outline">Sub-judul (Lokasi/Domisili)</label>
                            <input class="w-full p-md border border-outline-variant rounded-lg focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all bg-white" placeholder="Jakarta, Indonesia" type="text" name="subtitle" required value="{{ old('subtitle') }}">
                        </div>
                        <div class="space-y-base md:col-span-2">
                            <label class="font-label-md text-label-md text-outline">Tentang Saya</label>
                            <textarea class="w-full p-md border border-outline-variant rounded-lg focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all bg-white" placeholder="Ceritakan singkat tentang pengalaman dan minat profesional Anda..." rows="4" name="tentang_saya" required>{{ old('tentang_saya') }}</textarea>
                        </div>
                    </div>
                </div>
                        ...
                        @for ($i = 0; $i < 3; $i++)
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-md p-md bg-surface-container-low rounded-lg border border-dashed border-outline-variant">
                                <div class="space-y-base">
                                    ...
                                    <input class="w-full p-sm border border-outline-variant rounded-lg bg-white focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all" placeholder="{{ $pendidikanPlaceholders[$i]['tingkat'] }}" type="text" name="pendidikan[{{ $i }}][tingkat]" value="{{ old("pendidikan.{$i}.tingkat") }}">
                                </div>
                                <div class="space-y-base">
                                    <label class="font-label-md text-label-md text-outline">Institusi</label>
                                    <input class="w-full p-sm border border-outline-variant rounded-lg bg-white focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all" placeholder="{{ $pendidikanPlaceholders[$i]['universitas'] }}" type="text" name="pendidikan[{{ $i }}][universitas]" value="{{ old("pendidikan.{$i}.universitas") }}">
                                </div>
                                <div class="space-y-base">
                                    <label class="font-label-md text-label-md text-outline">Jurusan</label>
                                    <input class="w-full p-sm border border-outline-variant rounded-lg bg-white focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all" placeholder="{{ $pendidikanPlaceholders[$i]['jurusan'] }}" type="text" name="pendidikan[{{ $i }}][jurusan]" value="{{ old("pendidikan.{$i}.jurusan") }}">
                                </div>
                            </div>
                        @endfor
                    </div>
                            ...
                            @for ($i = 0; $i < 3; $i++)
                                <div class="flex gap-md items-end">
                                    <div class="flex-grow space-y-base">
                                        ...
                                        <input class="w-full p-sm border border-outline-variant rounded-lg bg-white focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all" placeholder="{{ $skillPlaceholders[$i]['skill'] }}" type="text" name="skill[{{ $i }}][skill]" value="{{ old("skill.{$i}.skill") }}">
                                    </div>
                                    <div class="w-24 space-y-base">
                                        <label class="font-label-md text-label-md text-outline">Level (%)</label>
                                        <input class="w-full p-sm border border-outline-variant rounded-lg bg-white focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all" placeholder="{{ $skillPlaceholders[$i]['kemampuan'] }}" type="number" min="1" max="100" name="skill[{{ $i }}][kemampuan]" value="{{ old("skill.{$i}.kemampuan") }}">
                                    </div>
                                </div>
                            @endfor
                        </div>
                            ...
                            @for ($i = 0; $i < 3; $i++)
                                <div class="flex gap-md items-end">
                                    <div class="flex-grow space-y-base">
                                        ...
                                        <input class="w-full p-sm border border-outline-variant rounded-lg bg-white focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all" placeholder="{{ $pengalamanPlaceholders[$i]['pengalaman'] }}" type="text" name="pengalaman[{{ $i }}][pengalaman]" value="{{ old("pengalaman.{$i}.pengalaman") }}">
                                    </div>
                                    <div class="w-32 space-y-base">
                                        <label class="font-label-md text-label-md text-outline">Durasi</label>
                                        <input class="w-full p-sm border border-outline-variant rounded-lg bg-white focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all" placeholder="{{ $pengalamanPlaceholders[$i]['durasi'] }}" type="text" name="pengalaman[{{ $i }}][durasi]" value="{{ old("pengalaman.{$i}.durasi") }}">
                                    </div>
                                </div>
                            @endfor
                        </div>
    ...
    <footer class="bg-primary dark:bg-tertiary-container border-t border-outline-variant/20">
        <div class="flex flex-col md:flex-row justify-between items-center px-margin-desktop py-lg w-full max-w-7xl mx-auto">
            <div class="mb-md md:mb-0">
                ...
                <p class="font-label-md text-label-md text-surface-variant mt-xs">© 2024 KerjaKuy. All rights reserved.</p>
            </div>
    ...
    <script>
        ...
        document.getElementById('cv-form').addEventListener('submit', (e) => {
            ...
            btn.innerHTML = '<span class="material-symbols-outlined animate-spin">refresh</span> <span>Menyimpan...</span>';
            btn.disabled = true;
            
            setTimeout(() => {
                btn.innerHTML = '<span class="material-symbols-outlined">check_circle</span> <span>Tersimpan!</span>';
