...
<head>
    ...
    <title>Pengaturan Akun Perusahaan - KerjaYok</title>
        ...
        <div class="flex justify-between items-center px-margin-mobile md:px-margin-desktop w-full">
            <div class="flex items-center gap-md md:gap-xl">
                ...
                <a href="/home-perusahaan"
                    class="text-headline-md font-headline-md font-bold text-on-surface">KerjaYok</a>
                <nav class="hidden md:flex items-center gap-lg">
                    <a class="font-body-md text-on-surface-variant hover:text-primary transition-colors"
                        href="/home-perusahaan">Lowongan Kerja</a>
                    <a class="font-body-md text-on-surface-variant hover:text-primary transition-colors"
                        href="/karyawanPerusahaan">Karyawan</a>
                    <a class="font-body-md text-on-surface-variant hover:text-primary transition-colors"
                        href="/perusahaan/wawancara">Wawancara</a>
                    <a class="font-body-md text-on-surface-variant hover:text-primary transition-colors"
                        href="{{ route('perusahaan.history') }}">History</a>
                </nav>
            </div>
            <div class="flex items-center gap-md">
                <a href="{{ route('perusahaan.settings') }}"
                    class="h-10 w-10 rounded-full border border-outline-variant overflow-hidden cursor-pointer hover:ring-2 hover:ring-secondary/20 transition-all flex items-center justify-center bg-secondary">
        ...
        <aside
            ...
            <nav class="flex-1 space-y-base px-sm">
                ...
                <a class="flex items-center gap-sm px-md py-sm rounded-lg bg-secondary text-white font-bold transition-all active:scale-95"
                    href="{{ route('perusahaan.settings') }}">
        ...
        <section class="flex-1 bg-surface py-lg px-margin-mobile md:py-xl md:px-margin-desktop overflow-y-auto">
            <div class="max-w-3xl">
                <header class="mb-lg border-b border-outline-variant/30 pb-md">
                    <a href="/home-perusahaan"
                        class="inline-flex items-center gap-2 text-on-surface-variant hover:text-primary mb-md transition-colors font-body-sm">
                ...
                <form action="{{ route('perusahaan.settings.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div
                        ...
                        <div class="relative group flex-shrink-0">
                            <div class="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-md cursor-pointer"
                                ...
                                @if ($perusahaan->foto_profil)
                                    <img id="profile-preview" alt="Profile Photo" class="w-full h-full object-cover"
                                        src="{{ asset('storage/' . $perusahaan->foto_profil) }}?t={{ time() }}" />
                                @else
                                    ...
                                    <img id="profile-preview" alt="Profile Photo"
                                        class="w-full h-full object-cover hidden" src="" />
                                @endif
                            </div>
                            ...
                            <input type="file" name="foto_profil" id="inputFoto" class="hidden" accept="image/*"
                                onchange="previewImg()" />
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-md">
                        ...
                        <div class="space-y-xs group">
                            ...
                            <div class="relative">
                                <input name="email"
                                    class="w-full h-12 px-md pr-12 bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface"
                                    type="email" value="{{ old('email', $perusahaan->email) }}" />
                                <button type="button"
                                    class="edit-btn absolute right-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-outline hover:text-secondary transition-colors cursor-pointer">edit</button>
                            </div>
                        </div>
                        ...
                        <div class="space-y-xs group">
                            ...
                            <div class="relative">
                                <input name="telepon"
                                    class="w-full h-12 px-md pr-12 bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface"
                                    type="text" value="{{ old('telepon', $perusahaan->telepon) }}" />
                                <button type="button"
                                    class="edit-btn absolute right-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-outline hover:text-secondary transition-colors cursor-pointer">edit</button>
                            </div>
                        </div>
                        <!-- Sektor Industri -->
                        <div class="space-y-xs group">
                            <label class="text-label-md font-label-md text-on-surface-variant px-base">Sektor
                                Industri</label>
                            <div class="relative">
                        ...
                        <div class="space-y-xs group">
                            ...
                            <div class="relative">
                                <input name="website"
                                    class="w-full h-12 px-md pr-12 bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface"
                                    type="text" value="{{ old('website', $perusahaan->website) }}" />
                                <button type="button"
                                    class="edit-btn absolute right-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-outline hover:text-secondary transition-colors cursor-pointer">edit</button>
                            </div>
                        </div>
                        <!-- NPWP (Read-Only) -->
                        <div class="space-y-xs group">
                            <label class="text-label-md font-label-md text-on-surface-variant px-base">NPWP (Tidak
                                dapat diubah)</label>
                            <div class="relative">
                                <input name="npwp"
                                    class="w-full h-12 px-md bg-surface-container/30 border border-outline-variant/50 rounded-lg outline-none font-body-md text-on-surface-variant cursor-not-allowed"
                                    type="text" value="{{ $perusahaan->npwp }}" readonly />
                            </div>
                        </div>
                        <!-- Lokasi / Alamat (Full Width) -->
                        <div class="space-y-xs group md:col-span-2">
                            <label class="text-label-md font-label-md text-on-surface-variant px-base">Lokasi /
                                Alamat</label>
                            <div class="relative">
                                <textarea name="lokasi" rows="3"
                                    class="w-full px-md py-sm bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface resize-none">{{ old('lokasi', $perusahaan->alamat) }}</textarea>
                            </div>
                        </div>
                        <!-- Deskripsi Perusahaan (Full Width) -->
                        <div class="space-y-xs group md:col-span-2">
                            <label class="text-label-md font-label-md text-on-surface-variant px-base">Deskripsi
                                Perusahaan</label>
                            <div class="relative">
                                <textarea name="deskripsi" rows="5"
                                    class="w-full px-md py-sm bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface resize-none">{{ old('deskripsi', $perusahaan->deskripsi) }}</textarea>
                            </div>
                        </div>
                    </div>
        ...
        <div
            ...
            <form action="{{ route('perusahaan.settings.updatePassword') }}" method="POST" class="space-y-md">
                ...
                <div class="space-y-xs">
                    ...
                    <div class="relative">
                        <input type="password" name="password_lama" data-password-input
                            class="w-full h-12 px-md pr-12 bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface"
                            required />
                        <button type="button" data-toggle-password
                            class="absolute right-0 top-0 h-12 w-12 flex items-center justify-center text-on-surface-variant hover:text-on-surface hover:bg-surface-container cursor-pointer transition-colors rounded"
                ...
                <!-- Password Baru -->
                <div class="space-y-xs">
                    <label class="text-label-md font-label-md text-on-surface-variant px-base">Password Baru (Min. 6
                        karakter)</label>
                    <div class="relative">
                        <input type="password" name="password_baru" data-password-input
                            class="w-full h-12 px-md pr-12 bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface"
                            required />
                        <button type="button" data-toggle-password
                            class="absolute right-0 top-0 h-12 w-12 flex items-center justify-center text-on-surface-variant hover:text-on-surface hover:bg-surface-container cursor-pointer transition-colors rounded"
                ...
                <div class="space-y-xs">
                    <label class="text-label-md font-label-md text-on-surface-variant px-base">Konfirmasi Password
                        Baru</label>
                    <div class="relative">
                        <input type="password" name="password_baru_confirmation" data-password-input
                            class="w-full h-12 px-md pr-12 bg-surface-container-lowest border border-outline-variant rounded-lg focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-on-surface"
                            required />
                        <button type="button" data-toggle-password
                            class="absolute right-0 top-0 h-12 w-12 flex items-center justify-center text-on-surface-variant hover:text-on-surface hover:bg-surface-container cursor-pointer transition-colors rounded"
