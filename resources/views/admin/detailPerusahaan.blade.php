...
<main class="max-w-5xl mx-auto px-margin-mobile md:px-margin-desktop py-lg flex-1 w-full">
    ...
    @if (session('success'))
        <div class="mb-md p-md rounded-lg bg-secondary-container text-on-secondary-container text-body-sm font-semibold border border-secondary/20 flex items-center gap-2">
            <span class="material-symbols-outlined text-[16px]">task_alt</span>
            <span>{{ session('success') }}</span>
        </div>
    @endif
    @if (session('error'))
        <div class="mb-md p-md rounded-lg bg-error-container text-on-error-container text-body-sm font-semibold border border-error/20 flex items-center gap-2">
    ...
    <!-- Back Link -->
    <a class="flex items-center gap-xs text-secondary font-label-md text-label-md mb-md hover:underline group" href="{{ route('admin.dashboard') }}">
    ...
    <div class="bg-surface-container-lowest rounded-xl border border-outline-variant/30 overflow-hidden shadow-sm">
        ...
        <div class="hero-gradient px-lg py-xl text-white relative">
            <div class="absolute top-0 left-0 w-full h-full opacity-10 overflow-hidden pointer-events-none">
                <div class="absolute -top-1/2 -left-1/4 w-[150%] h-[200%] rotate-12 bg-gradient-to-b from-white/20 to-transparent"></div>
            </div>
            <div class="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-6 max-w-4xl mx-auto text-center md:text-left">
                <!-- Foto Profil di Samping Kiri Nama Perusahaan -->
                <div class="w-24 h-24 md:w-32 md:h-32 rounded-2xl border-4 border-white/20 overflow-hidden bg-white/10 flex items-center justify-center flex-shrink-0 shadow-lg">
                    ...
                    @else
                        <span class="material-symbols-outlined text-white text-[48px] md:text-[64px]" style="font-variation-settings: 'FILL' 0;">business</span>
                    @endif
                </div>
                <div class="flex-grow py-2">
                    <h1 class="font-headline-xl text-headline-xl mb-xs leading-tight break-words">{{ $perusahaan->nama_perusahaan }}</h1>
                    <p class="font-body-md text-body-md text-white/80 font-medium tracking-wide uppercase">{{ $perusahaan->sektor_industri ?? 'Sektor Industri Belum Ditentukan' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-lg space-y-lg">
            ...
            <section>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-y-md gap-x-lg">
                    <div class="break-words">
                        <p class="font-label-sm text-label-sm text-outline uppercase mb-xs tracking-wider">Nama Perusahaan</p>
                        <p class="font-body-md text-body-md text-on-surface font-semibold">{{ $perusahaan->nama_perusahaan }}</p>
                    </div>
                    <div class="break-words">
                    ...
                    <div class="break-words">
                        <p class="font-label-sm text-label-sm text-outline uppercase mb-xs tracking-wider">Telepon</p>
                    ...
                    <div class="break-words">
                        <p class="font-label-sm text-label-sm text-outline uppercase mb-xs tracking-wider">Alamat</p>
                    ...
                    <div class="break-words">
                        <p class="font-label-sm text-label-sm text-outline uppercase mb-xs tracking-wider">Tanggal Pendaftaran</p>
                        <p class="font-body-md text-body-md text-on-surface">{{ $perusahaan->created_at->format('d/m/Y H:i') }}</p>
                    </div>
                </div>
            </section>
            ...
            <section>
                <div class="flex items-center gap-sm pb-sm border-b border-outline-variant/40 mb-md">
                    <span class="material-symbols-outlined text-secondary" style="font-variation-settings: 'FILL' 1;">folder_open</span>
                    <h2 class="font-headline-md text-headline-md text-primary">Dokumen</h2>
                </div>
                <div class="grid grid-cols-1 gap-md">
                    <!-- Sertifikat -->
                    <div class="flex items-start gap-md p-md bg-surface-container-low rounded-lg border border-outline-variant/20 hover:border-secondary/30 transition-all cursor-pointer group">
                        <div class="w-12 h-12 bg-secondary/10 rounded-lg flex items-center justify-center text-secondary group-hover:bg-secondary group-hover:text-white transition-colors">
                            <span class="material-symbols-outlined">verified_user</span>
                        </div>
                        <div>
                            <p class="font-label-sm text-label-sm text-outline uppercase mb-xs tracking-wider">Sertifikat Perusahaan</p>
                            @if($perusahaan->sertifikat)
                                <a class="flex items-center gap-xs text-secondary font-label-md text-label-md font-bold" href="{{ asset('storage/' . $perusahaan->sertifikat) }}" target="_blank">
                                    <span class="material-symbols-outlined text-[20px]">download</span>
                                    Download Sertifikat
                                </a>
                            @else
                                <p class="font-body-md text-body-md text-on-surface-variant italic">Belum ditambahkan</p>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <section>
                <div class="flex items-center gap-sm pb-sm border-b border-outline-variant/40 mb-md">
                    <span class="material-symbols-outlined text-secondary" style="font-variation-settings: 'FILL' 1;">fact_check</span>
                    <h2 class="font-headline-md text-headline-md text-primary">Status Verifikasi</h2>
                </div>
                <div class="flex flex-col gap-xs">
                    <p class="font-label-sm text-label-sm text-outline uppercase tracking-wider">Status Saat Ini</p>
                    @if($perusahaan->status_verifikasi === 'pending')
                        <div class="inline-flex items-center gap-xs px-md py-xs bg-amber-50 text-amber-700 border border-amber-200 rounded-full w-fit">
                    ...
                    @elseif($perusahaan->status_verifikasi === 'verified')
                        <div class="inline-flex items-center gap-xs px-md py-xs bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full w-fit">
                            <span class="material-symbols-outlined text-[18px]">check_circle</span>
                            <span class="font-label-md text-label-md font-bold">Terverifikasi</span>
                        </div>
                    @else
                        <div class="inline-flex items-center gap-xs px-md py-xs bg-rose-50 text-rose-700 border border-rose-200 rounded-full w-fit">
                    ...
                    @if ($perusahaan->alasan_penolakan)
                        <div class="mt-md p-md bg-rose-50/50 rounded-lg border border-rose-100 max-w-xl">
                            <p class="font-label-sm text-rose-800 uppercase mb-xs tracking-wider">Alasan Penolakan</p>
                ...
                <section class="bg-secondary-container/10 p-md rounded-xl border border-secondary/10">
                    ...
                    <form action="{{ route('admin.verifikasiPerusahaan', $perusahaan->id) }}" method="POST">
                        ...
                        <div class="space-y-md">
                            ...
                            <div class="flex flex-wrap gap-lg">
                                <label class="flex items-center gap-sm cursor-pointer group">
                                    <div class="relative flex items-center justify-center">
                                        <input class="w-6 h-6 border-outline text-secondary focus:ring-secondary cursor-pointer" name="status_verifikasi" type="radio" value="verified" id="status_verified" required onchange="toggleRejectSection()" {{ old('status_verifikasi') == 'verified' ? 'checked' : '' }}>
                                    </div>
                                    <div class="flex items-center gap-xs">
                                        <span class="material-symbols-outlined text-green-600 font-bold text-[20px]">check</span>
                                        <span class="font-body-md text-body-md text-on-surface">Verifikasi (Setujui)</span>
                                    </div>
                                </label>
                                <label class="flex items-center gap-sm cursor-pointer group">
                                    <div class="relative flex items-center justify-center">
                                        <input class="w-6 h-6 border-outline text-error focus:ring-error cursor-pointer" name="status_verifikasi" type="radio" value="rejected" id="status_rejected" onchange="toggleRejectSection()" {{ old('status_verifikasi') == 'rejected' ? 'checked' : '' }}>
                                    </div>
                                    <div class="flex items-center gap-xs">
                                        <span class="material-symbols-outlined text-error font-bold text-[20px]">close</span>
                            ...
                            <div class="{{ old('status_verifikasi') == 'rejected' ? '' : 'hidden' }}" id="rejectSection">
                                <div class="flex flex-col gap-xs mt-md">
                                    <label for="alasan_penolakan" class="font-label-md text-label-md text-on-surface font-bold">Alasan Penolakan (Wajib jika ditolak):</label>
                                    <textarea id="alasan_penolakan" name="alasan_penolakan" rows="3" class="w-full max-w-xl border border-outline-variant text-on-surface rounded-xl py-2 px-3 focus:ring-2 focus:ring-error focus:border-error" placeholder="Jelaskan alasan penolakan...">{{ old('alasan_penolakan') }}</textarea>
                                </div>
                            </div>

                            <div class="flex flex-wrap gap-md pt-md">
                                <button type="submit" class="px-xl py-sm bg-secondary text-white rounded-lg font-label-md text-label-md hover:opacity-90 active:scale-95 transition-all shadow-md shadow-secondary/20">
                                    Simpan Keputusan
                                </button>
                                <a href="{{ route('admin.daftarPerusahaan') }}" class="px-xl py-sm bg-outline-variant text-on-surface-variant rounded-lg font-label-md text-label-md hover:bg-outline-variant/70 active:scale-95 transition-all text-center">
                                    Batal
                                </a>
                            </div>
                        </div>
                ...
                <section class="bg-surface-container-low p-md rounded-xl border border-outline-variant/20">
                    <p class="text-body-md text-on-surface-variant font-medium">
                        ...
                        @if ($perusahaan->verified_at)
                            <br><span class="text-xs text-outline mt-1 block">Diproses pada: {{ \Carbon\Carbon::parse($perusahaan->verified_at)->format('d/m/Y H:i') }}</span>
                        @endif
                    </p>
                </section>
            @endif
